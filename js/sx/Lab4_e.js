...
//
var colors = vec4.fromValues(0.235, 0.505, 0.535, 1.0);
...
function addEvent() {
    ...
    document.getElementById("_color").onclick = function(event) {
        ...
        var G = parseInt(color[5], 16) * 16 + parseInt(color[4], 16);
        var B = parseInt(color[7], 16) * 16 + parseInt(color[6], 16);
        R /= 255.0;
        G /= 255.0;
        B /= 255.0;
...
function setCircle(position) {
    ...
    scaleCircle.push(vec3.fromValues(0.15, 0.15, 1.0));
    thetaCircle.push(vec3.fromValues(0.0, 0.0, 0.0));
    speedOffset.push(vec3.fromValues( Math.sin(angle), Math.cos(angle), 0) );
    flagX.push(1);
    flagY.push(1);
    var cnt = 0;
    gl.bindBuffer(gl.ARRAY_BUFFER, vBuffer);
